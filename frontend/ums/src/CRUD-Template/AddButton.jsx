...
export default function AddButton ({setConfirmExit, renderBot}) {
  ...
  const getId = async (id) => {
    if (id > 0){
      try {
        const fetchProfessor = await axios.get(`http://localhost:8080/professor/getProfessor/${id}`);
        handleChange(fetchProfessor.data);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8080/professorLenda/createLigjerata', JSON.stringify(formData), {
        headers: {
